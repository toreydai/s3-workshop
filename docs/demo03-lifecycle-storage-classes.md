...
本实验完整覆盖了 S3 的成本优化工具链：通过生命周期规则自动化"热→温→冷→删除"数据分层，通过 Intelligent-Tiering 处理不可预测访问模式，通过 NoncurrentVersionExpiration 清理版本历史避免版本控制的隐性成本，最后用同区域复制（SRR）为关键数据建立同账号、同区域的独立副本，满足数据隔离和合规备份需求。SRR 与 Demo04 的 CRR 合力构成完整的 S3 复制体系。
