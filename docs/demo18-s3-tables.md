...
- **权限**：托管策略 `AmazonS3TablesFullAccess`，另需 Athena 查询权限（`AmazonAthenaFullAccess`）和 Glue Catalog 权限（`AWSGlueConsoleFullAccess`）
  > ⚠️ 如果账号已经启用过 Lake Formation（存在显式 `DataLakeAdmins` 列表），`glue create-catalog`/`glue get-catalog` 会额外要求调用者是 **Lake Formation Data Lake Administrator**，即使该角色本身拥有 IAM 管理员权限也会被拒绝（`AccessDeniedException: Insufficient Lake Formation permission(s): Required Create Catalog on Catalog`）。这是 Lake Formation 独立于 IAM 的权限模型，标准托管策略不覆盖。修复：把当前执行角色加入 Data Lake Admins，例如
  > ```bash
  > aws lakeformation put-data-lake-settings --region us-east-1 --data-lake-settings "{\"DataLakeAdmins\": [{\"DataLakePrincipalIdentifier\": \"$(aws sts get-caller-identity --query Arn --output text | sed 's#assumed-role/\([^/]*\)/.*#role/\1#')\"}]}"
  > ```
  > （合并已有的 admin 列表，不要覆盖）。全新的、从未配置过 Lake Formation 的账号通常不会遇到这个问题。
- **初始化**：
...
CREATE_CATALOG_OUTPUT=$(aws glue create-catalog \
...
if echo "${CREATE_CATALOG_OUTPUT}" | grep -q "AlreadyExistsException"; then
  echo "s3tablescatalog 已存在，跳过"
elif echo "${CREATE_CATALOG_OUTPUT}" | grep -q "ERROR"; then
  echo "${CREATE_CATALOG_OUTPUT}"
  exit 1
fi
...
> ⚠️ 原先用 `grep -v "AlreadyExistsException" || echo ...` 判断幂等性的写法有 bug：`aws` CLI 在报错前会输出一个空行，`grep -v` 会把这个空行当作"未匹配到 AlreadyExistsException 的正常内容"打印出来，导致 `grep` 退出码是 0、`||` 分支永远不会触发，脚本不报错但也不会给出"已存在，跳过"的提示。已改为用 `grep -q` 显式匹配错误信息内容判断，而不是反向过滤。
